...
import { HeartPulse, Activity, Heart, Moon, TrendingUp, TrendingDown, Zap } from 'lucide-react';
...
function MetricCompareRow({ icon, label, value, avg, unit, lowerIsBetter }) {
  ...
  const diff = Math.abs(value - avg).toFixed(1);
  return (
    <div style={{
...
export default function ReadinessCard({ readiness }) {
  ...
  const tierBg = {
    PUSH: 'rgba(34,197,94,0.1)',
    MODERATE: 'rgba(245,158,11,0.1)',
    RECOVER: 'rgba(239,68,68,0.1)',
  };

  const tierBorder = {
    PUSH: 'rgba(34,197,94,0.2)',
    MODERATE: 'rgba(245,158,11,0.2)',
    RECOVER: 'rgba(239,68,68,0.2)',
  };

  const tierMessages = {
    PUSH: `Score ${score} — You're recovered. Push hard in training today. Good window for match-intensity work and PR attempts.`,
    MODERATE: `Score ${score} — Train but don't max out. Technical work over match intensity. Skip PR attempts in the gym.`,
    RECOVER: `Score ${score} — Prioritize recovery. Light movement only. No high-intensity training today.`,
  };
  ...
  return (
    <>
      <div
        className="card"
        style={{ borderColor: getCardBorderColor(score) }}
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{
              ...
              color: tierColor, marginBottom: 12
            }}>
              {tier}
            </div>
            <div style={{ display: 'flex', gap: 16 }}>
              {todayData && (
                <>
                  <MetricPill label="HRV" value={`${Math.round(todayData.hrv)}`} color="var(--accent-purple-light)" />
                  <MetricPill label="RHR" value={`${Math.round(todayData.rhr)}`} color="var(--accent-blue-light)" />
                  <MetricPill label="Sleep" value={`${todayData.sleep.toFixed(1)}h`} />
                </>
              )}
            </div>
          </div>
      ...
      <DetailDrawer open={open} onClose={() => setOpen(false)}>
        <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 20 }}>Readiness</h2>
        ...
        {todayData && (
          <div style={{ marginBottom: 20 }}>
            <MetricCompareRow
              ...
              label="HRV"
              value={Math.round(todayData.hrv)}
              avg={Math.round(todayData.hrv7dAvg)}
              unit="ms"
              lowerIsBetter={false}
            />
            <MetricCompareRow
              icon={<Heart size={14} />}
              label="Resting HR"
              value={Math.round(todayData.rhr)}
              avg={Math.round(todayData.rhr7dAvg)}
              unit="bpm"
              lowerIsBetter={true}
            />
            <MetricCompareRow
              ...
              avg={todayData.sleep7dAvg.toFixed(1)}
              unit="h"
              lowerIsBetter={false}
            />
            <MetricCompareRow
              icon={<Moon size={14} />}
              label="Deep + REM"
              value={todayData.deepRem.toFixed(2)}
              avg={todayData.deepRem7dAvg.toFixed(2)}
              unit="h"
              lowerIsBetter={false}
            />
          </div>
        ...
        <div style={{
          ...
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
            <strong style={{ color: tierColor }}>{tier}:</strong> {tierMessages[tier].slice(tier.length + 4)}
          </p>
        </div>
