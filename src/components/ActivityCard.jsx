...
import { Footprints, Dumbbell, Flame, BedDouble } from 'lucide-react';
...
function StepRing({ steps, goal = STEP_GOAL, size = 80 }) {
  ...
  return (
    <svg width={size} height={size}>
      ...
      <text x={cx} y={cy - 4} textAnchor="middle" dominantBaseline="middle"
        style={{ fontSize: 14, fontWeight: 700, fill: 'var(--text-primary)', fontFamily: 'var(--font-display)' }}>
  ...
  return (
    <>
      ...
      <DetailDrawer open={open} onClose={() => setOpen(false)}>
        <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 20 }}>Activity</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
          ...
          <StatBox icon={<Flame size={14} color="var(--text-secondary)" />} label="Total Energy Out" value={`${Math.round(activeEnergy + restingEnergy)} kcal`} />
        </div>
